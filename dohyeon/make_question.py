...
def parse_content(page_content):
    ...
    try:
        question_text = page_content.split('dataSet.question.raw.text:')[1].strip()
    except IndexError:
...
def make_D_I_template(question, tokenizer, model, device, index,  docs,keyword,card):
    ...
    for keyword in keywords:
        ...
        combined_vector = (0.3 * query_vector + 0.7 * keyword_vector) 
        # 결합된 벡터로 문서 검색
...
def search_and_respond(question, model_id, docs, D, I, template, max_gpus=8):
...
def main(question,card:str='ARD'):
    ...
    index = faiss.read_index("/home/ubuntu/dohyeon/db_indexs.faiss")
    doc_vectors = np.load("/home/ubuntu/dohyeon/doc_vectors.npy")
    loader = CSVLoader(file_path="/home/ubuntu/dohyeon/data_sample.csv")
    docs = loader.load()
...
if __name__ == "__main__":
    question = 'PS 직무의 직무면접에 참가하게 된 신입 지원자입니다. 십 년 후에 제가 생각하는 저의 미래는 어떨 것 같으신가요 어떤 자신감 있는 외모와 어떤 자신감 있는 외모를 가진지 귀여운 아이를 낳는지는 모르겠습니다만 훌륭한 부모를 맡아보는 것 같습니다'
