...
$(document).ready(function() {
    ...
    $("#type-select").change(function() {
        ...
        // 섹션 가시성과 required 속성 업데이트
        if (selectedValue === "personal_statement_form") {
            $("#personal-statement-section").show();
            $("#project-section").hide();
            // Required 속성 업데이트
            $('#introduction-article, #personal_statement').prop('required', true);
            $('#project_title, #project_description').prop('required', false);
        } else if (selectedValue === "project") {
            $("#personal-statement-section").hide();
            $("#project-section").show();
            // Required 속성 업데이트
            $('#project_title, #project_description').prop('required', true);
            $('#introduction-article, #personal_statement').prop('required', false);
        }
    });

    // 초기 상태에 맞게 change 이벤트를 강제 실행
    $("#type-select").change();

    // 폼 제출 처리
    $('#personal-statement-form').submit(function(e) {
        e.preventDefault();
        var allValid = this.checkValidity(); // 폼 유효성 검사
        ...
        if (!allValid) {
            ...
            $("#loading-message").show();
            // 예시 AJAX 호출
            $.post('/generate_questions', $(this).serialize(), function(data) {
                // 응답 처리
